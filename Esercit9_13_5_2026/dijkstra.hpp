...
template<typename T> //restituisco T = quanto è il peso totale alla fine del Single Source Shortest Path
void dijkstra(const undirected_graph<T>& grafo, const T& source, std::map<undirected_edge<T>, double> weights, std::map<T, T>& predecessori, std::map<T, double>& distanze) {//, w = vettore dei pesi associati agli archi?
    ...
    // Glieli passo SOLO per riferimento se no il main non funziona!:
    ...
    while (!PQ.empty()) {
        ...
        PQ.erase(PQ.begin()); // e poi lo tolgo

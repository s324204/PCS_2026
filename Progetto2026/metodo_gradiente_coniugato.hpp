...
Eigen::Vector<T, Eigen::Dynamic> gradiente_coniugato(const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& A, Eigen::Vector<T, Eigen::Dynamic>& b, unsigned int& it, const unsigned int& it_max, const double& tol, const double& res_tol) {
  ...
  int dim=A.cols();
  it = 0;
  Eigen::Vector<T, Eigen::Dynamic> x = Eigen::Vector<T, Eigen::Dynamic>::Zero(dim); // il nostro x0 di partenza
...
//avrei potuto usare i metodi nativi delle Eigen come .dot() (trovati su video youtube e internet) per evitare .transpose() e .value() e 
//migliorare le prestazioni, ma ho voluto rispettare cioè che aveva fatto il professore in aula